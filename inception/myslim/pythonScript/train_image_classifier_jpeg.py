...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      neg_M = tf.multiply(tf.expand_dims(neg_ls,1), 
                          tf.ones([FLAGS.batch_size, dataset.num_classes - FLAGS.labels_offset], dtype=tf.float32))
      
      onehot_tmp = tf.multiply(tf.expand_dims(tf.subtract(t_ps, neg_ls),1),
